...
> Chapter 2.5 · Transfers

![发送 USDT 前，我会按这张清单过一遍](../site/assets/css/send-checklist.svg)

USDT 转账看起来就是复制地址、填金额、确认。真正把人坑进去的，往往是网络、地址、资源、没做小额测试这几件事同时出问题。

> TL;DR：发 USDT 前先看四件事——对方支持什么网络、地址对不对、钱包资源够不够、是不是第一次给这个地址打款。是第一次，就先打一笔小额。

> 延伸：如果你是被 `memo / tag / label / address alias` 这些词搞混，读 [USDT 的 Memo、Tag、Label、Address Alias 是什么](./usdt-memo-tag-label-address-alias.md)；如果你是被 `flash usdt` 这种词吓到，读 [什么是 Flash USDT，为什么它基本都和骗局有关](./what-is-flash-usdt-scam.md)。

## 为什么“网络”比“币名”更重要


**「网络比币名重要」指的是**：USDT 同时存在于 TRC20、ERC20 等多条链，币名相同不代表路线兼容，选错链会让资金到不了对方账户。
...
**发送前要检查的 5 件事**是：对方支持的网络、地址是否完整准确、金额是否正确、钱包资源够不够、是不是第一次给这个地址转账。
...
**我的默认检查顺序**是：先看链，再看地址，再看金额和规则，最后看钱包资源够不够——按这个顺序过一遍能避开绝大多数「差最后一步才发现漏看」的事故。
...
**常见发送流程**分四步：先看对方的收款规则（链、备注、金额、时限），在钱包里选对资产和网络，复制地址后至少核对两遍，第一次先小额测试。
...
**发送环节最容易踩的坑**集中在几处：只看 USDT 不看网络、复制了错误或旧地址、第一次直接大额、忽略 TRON 资源、漏看对方要求的 memo / tag。
...
- 补充：[USDT 的 Memo、Tag、Label、Address Alias 是什么](./usdt-memo-tag-label-address-alias.md)
- 安全：[什么是 Flash USDT，为什么它基本都和骗局有关](./what-is-flash-usdt-scam.md)
