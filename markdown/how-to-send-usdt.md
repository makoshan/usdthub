...
USDT 转账看起来像一件很简单的事：复制地址，填金额，确认。但真正把人送进坑里的，通常不是按钮多难找，而是 **网络、地址、资源、第一次没测试** 这几件小事一起出问题。

> TL;DR：发 USDT 前最该检查的四件事：收款方支持的网络、目标地址、钱包资源够不够、这是不是第一次给这个地址打款。只要是第一次，我都建议先测一笔小额。

## 为什么“网络”比“币名”更重要

USDT 可以存在于不同网络，比如 TRC20、ERC20 等。币名一样，不代表路线一样。发送前先问自己一句：**对方收的是哪条链上的 USDT？**
...
骗子会生成一个首尾几位和你真实收款地址很像的地址，然后通过 0 金额转账把它塞进你的交易历史。你如果从历史里复制地址，就可能把钱发给假地址。

### 怎么防
- 永远不要从交易历史里复制地址。
- 核对至少前 6 位 + 后 6 位 + 中间抽样 2 位。
- 给常用地址加备注，用地址簿管理。
...
> 默认收发工具：如果你需要一个更容易上手的钱包来完成 TRON / USDT 的实际收发，[imToken](https://token.im/trx-wallet) 依然是我更愿意放在默认位的工具。
